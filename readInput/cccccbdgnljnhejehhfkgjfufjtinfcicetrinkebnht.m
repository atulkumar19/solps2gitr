...
for i=1:nBins
    ...
    hist_bins(i) = sum(weight0(find(z0>=0.1999 & sqrt(x0.^2+y0.^2) > edges(i) & sqrt(x0.^2+y0.^2) <= edges(i+1))));
...
DownstreamAverageFlux(4);
% line averaged downstream flux plot
...
c=colorbar;
h.EdgeColor = 'none';
title('Al 1+ Impurity Density at Z=0.2 m')
xlim([-0.06 0.06])
ylim([-0.06 0.06])
xlabel('X [cm]')
ylabel('Y [cm]')
